...
OLD_SAMPLE_PATH = data_folder + "processed/annotations/sampled_teacher_utt.csv"
OUTPUT_PATH = data_folder + "processed/annotations/sampled_teacher_utt_pseudolabels.csv"
...
# Extract OBSID and utterance indexp
...
# --------------------
# Load old samples and exclude them
# --------------------
old_sample = pd.read_csv(OLD_SAMPLE_PATH)

# Old unique IDs (these correspond to comb_idx)
old_ids = set(old_sample["target_comb_idx"].astype(str))

# Only teacher utterances
teacher_df = df[df["speaker"] == "teacher"]

# Exclude previously sampled utterances
teacher_df = teacher_df[~teacher_df["comb_idx"].astype(str).isin(old_ids)]

print(f"Available teacher utterances after exclusion: {len(teacher_df)}")

# --------------------
# Sample new teacher utterances
# --------------------
...
print(f"Saved {len(out_df)} new centered examples to {OUTPUT_PATH}")